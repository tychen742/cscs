```{index} single: array; one dimensional
```
...
`element type` of the array. [^arr-microsoft]
...
them using array indices. [^arr-codeacademy]
...
```{index} single: [ ]; array indexing array; indexing [ ] index; array
```
...
```{index} array; as parameter example; PrintStrings
```
...
```{index} function; return array array; returned by method
```
...
```{index} command line; parameter parameter; command line to Main Main; parameters
```
...
```{index} example; Scale Scale example array; parameter
```
...
```{index} OOP; default value default value in instance
```
...
[^arr-microsoft]: See [Arrays](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-specification/arrays) in Microsoft Langauge Reference.

[^arr-codeacademy]: See [C# Arrays](https://www.codecademy.com/learn/learn-c-sharp/modules/learn-csharp-arrays/cheatsheet) of codeacademy.
