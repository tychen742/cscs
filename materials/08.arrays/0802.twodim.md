...
```{index} array; two dimensional two dimensional array
```

An array can have any number of dimensions. For example, one-dimension (1D), 2-dimension
(2D), 3-dimension (3D) as shown below [^multi-d-arrays].
...
```{index} single: array; of arrays single: array; two-dimensional, ragged
```
...
[^multi-d-arrays]: Note this [figure](https://indianaiproduction.com/python-numpy-array/) is an illustration of arrays for NumPy
