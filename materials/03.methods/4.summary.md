...
1. The general syntax for defining a static method is
...
2. The *parameter list* can be empty or contain one or more comma separated
   *parameter* entries.
   Each parameter entry has the form

   > **type** **parameterName**

3. If the method is going to be called from outside its class, the heading needs
   to start with `public` before the `static`.

4. If **returnTypeOrVoid** in the heading is not `void`, there must be a
...
5. Execution of a program starts at a method with a heading including
