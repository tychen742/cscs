...
06. The class name of this project is *Ch09CollectionsLab*.
07. When executing code, you will start with the Main() method in a designated class/file.
08. You will prepare methods in the same class or project to be called from the Main() method.
...
1. fake_help_verbose ({repsrc}`fake_help_verbose`),
2. fake_help.cs ({repsrc}`dict_lab_stub`) along with the following files:
3. file_util.cs
4. help_not_defaults.txt
5. help_not_responses.txt, and
6. the UI class (`ui.cs` at {repsrc}`ui`) so you don't have to write the user input code.
