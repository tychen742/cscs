...
```{index} dictionary; key lookup [ ] single: [ ]; dictionary key lookup
```
...
```{index} example; ContainsKey dictionary; ContainsKey example ContainsKey example
```
