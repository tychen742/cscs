...
```{index} single: List; constructor single: List; Count single: List; Add single: List; Remove single: List; RemoveAt single: List; Contains
```
...
```{index} list; index [ ] single: [ ]; list index
```
...
```{index} single: List; constructor with sequence
```
...
```{index} example; ReadLines ReadLines example List; ReadLines example
```
