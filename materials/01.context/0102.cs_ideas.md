...
1. Describing a problem scenario to define the goals and objectives.
2. Plan and organize the features of the computer program that would solve the problem.
3. Writing the *source code* of the program using code editors.
4. Compile the source code into object code as a standalone executable program, usually with the help of a Software Development Kit (SDK) for linking with required resources.
5. Execute the program and debug to ensure the program runs as intended.
...
```{index} single: programming constructs; sequential-execution
```
...
vocabulary of programming, including terms like statement, operator, expression, value, and type [^f1].
...
of thinking; as Allen Downey [^f1] puts it, to "think like a computer scientist."
...
behavior of complex systems, form hypotheses, and test predictions" [^f1].
...
```{index} bit byte binary number system base 2 hexadecimal number system
```
...
An interpreter processes the program a little at a time, alternately reading lines and performing computations. [^f3]
:::

:::{figure} ../../images/compiler.jpg
:width: 50%

A compiler translates source code into object code, which is run by a hardware executor. [^f3]
:::
