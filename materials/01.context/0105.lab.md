...
substitutions:
  vscode-terminal: |-
    ```{image} ../../images/vscode-terminal.jpg
    :height: 2ex
    ```
  vscode-toggle: |-
    ```{image} ../../images/vscode-toggle.jpg
    :height: 2ex
    ```
---

```{index} Visual Studio Code, VS Code labs; VS Code
```
...
A Tutorial [^footnote-1]. In the book, it says "[t]he only ways to learn
...
1. Create workspace folders:
   ...
   :::{note}
   You can perform this task using your computer's GUI but this is an important
   practice, especially if you are new to the terminal/shell.
   :::

   01. Start your terminal application (Windows PowerSell or macOS Terminal): By default,
       you would be in your **user home directory** with a command prompt when the terminal app starts.

   02. Issue the command `pwd` to show your present work path to make sure you are in your user home directory.
   ...
   03. Type the `ls` command + Enter to see the files and directories in your user home directory.
       You should see the usual folder/directory names.

   04. Create a directory called "workspace" (`mkdir workspace`) in your user home directory if you have not done so.

   05. Change directory into the workspace directory (`cd workspace`).

   06. Inside workspace, create a directory **introcscs** (`mkdir introcscs`).

   07. Also inside workspace, create another directory called tests (`mkdir tests`) if you have not done so.

   08. `ls` + Enter to make sure that you have two directories created: introcscs and tests.

   09. Change directory up one level (`cd ..`) or just `cd ~` to go back to the your user home directory.
...
2. Create and change directory into a project folder:
   ...
   2. Change directory (`cd`) into the **tests** directory: The tests directory is inside
   ...
   3. Make sure you are in *tests*: `pwd` + Enter and you should see that you are in the
