```{index} single: &&; boolean operation AND and && boolean operation; && AND
```
...
```{index} single: ||; boolean operation OR or || boolean operation; || OR
```
...
```{index} single: ! boolean operation NOT not ! boolean operation; ! NOT
```
