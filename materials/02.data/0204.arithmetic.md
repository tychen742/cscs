...
```{index} single: operator; + with numbers single: +; with numbers
```
...
```{index} operator; - single: -; subtraction
```
...
```{index} operator; * single: * multiplication
```
...
```{index} single:  ( ); grouping grouping ( ) single: -; negation
```
...
```{index} single: remainder % single: % remainder single: operator; /, % division single: / ; division single: . ; double literal double int type; int type; double
```
