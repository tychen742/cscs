...
```{index} string; concatenation with + concatenation single: +; string concatenation operator; + string concatenation
```
...
```{index} escape code \ single: \ ; character escape code character escape code \
```
