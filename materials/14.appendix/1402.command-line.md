...
```{index} command line; paths paths on command line
```
...
: 1. Note: this approach does *not* give your a Mono command prompt.
  2. In Windows explorer navigate to the parent folder,
     showing the folder you want as a subfolder.
  3. Hold down the shift key and *right* click on the desired folder.
...
```{index} command line; dir and ls dir on Windows command line ls on Mac command line
```
...
```{index} command line; cd cd on command line change directory on command line
```
...
```{index} command line; mkdir mkdir on command line
```
...
```{index} command line; rmdir rmdir on command line
```
...
```{index} command line; display text file type on Windows command line cat on Mac command line
```
...
```{index} command line; copy file copy on Windows command line cp on Mac command line
```
...
```{index} command line; delete a file erase on Windows command line rm on Mac command line
```
...
```{index} command line; help help on command line man on Mac command line
```
...
```{index} command line; script script on command line
```
...
```{index} command line; copy and paste text copy text on command line paste text on command line
```
...
```{index} command line; shortcuts shortcuts on command line file completion on command line history on command line
```
...
[^ryacobellis]: Thanks to Dr. Robert Yacobellis for elaborations to this section.
