```{index} labs; hg and version control version control lab hg
```
...
6. How to get our book, examples, and projects (now that that you know how
   to use Mercurial).
...
1. Using your text editor, create a file `.hgignore`. You can simply
...
1. Create or copy your existing `Hello World` example, hello.cs to the
   the `labs` folder.

2. Let's test whether .hgignore is having any effect. Go to the
   `labs` folder and compile the `Hello, World.` example.

3. Verify that the .cs and .exe files are in the labs directory
...
6. Commit the changes, and then see the log entry
...
7. Push the changes to Bitbucket with the following command.
...
1. copy in or create a simple program in a directory you created,
   like labs/hello.cs
...
4. Add the new solution/projects to Mercurial. At this point, if the above list looks
...
2. Go to Repositories and look for your repository. In the authors case, it is under
   `gkthiruvathukal / gkt170`.

3. It pays to take a quick look at the dashboard. You'll see the *recent commits* on the
...
4. You can click on any revision to see what changes were made. It is ok to do so at this
...
5. If all was done properly, you will see .hgignore and labs. These
...
1. Pick a different location (outside of your repository folder and its subfolders)
...
2. Don't worry about breaking anything. Because Bitbucket knows what users are allowed
...
4. For example, if you performed a clone to introcs-csharp-examples, you should be able to change directory to introcs-csharp-examples:
