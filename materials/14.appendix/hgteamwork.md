```{index} Mercurial version control
```
...
1. Make sure your repository is selected. This is always the
...
4. Add the Email or Email Diff service. These services are basically
   equivalent, but one will generate links so you can view the
   differences that were just pushed. We recommend Email Diff.
5. Add the email notification address. You can only have one address.
