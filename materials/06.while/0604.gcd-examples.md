```{index} algorithms; greatest common divisor Euclid's algorithm greatest common divisor algorithm
```
...
follows, where $a \bmod b$ means `a % b` in C#.
...
`GreatestCommonDivisor` function. [^gcdbrute]
...
- The *right order* means that $a > b$. As we noted earlier, the
...
1. `b`, in which case `b` exactly divided the earlier `a`,
   and `b` is the GCD, or
2. a number less than `b`, namely
   $a \bmod b$ (or in C# terms `a % b`), and the process continues....
...
```{index} double:  recursion; greatest common divisor
```
...
[^gcdbrute]: The original brute-force gcd approach always goes through all the integers
