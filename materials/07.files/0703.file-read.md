```{index} file (StreamWriter); read and close StreamReader; ReadLine close file
```
...
```{index} StreamReader; EndOfStream EndOfStream
```
...
```{index} StreamReader; null from ReadLine ReadLine; null with StreamReader
```
...
```{index} file (StreamWriter); ReadToEnd StreamReader; ReadToEnd ReadToEnd
```
