...
```{index} example; sum_files.cs sum_files.cs example
```
...
```{index} File class; Exists Exists - File class method
```
...
```{index} exercise; safe sum safe sum exercise
```
...
1. From {repsrc}`sum_file/sum_file.cs`, copy the method CalcSum
