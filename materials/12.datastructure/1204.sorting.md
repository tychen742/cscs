```{index} PF4 recursion SP1 history sorting
```
...
```{index} sorting; bubble sort algorithms; bubble sort array; nested loop bubble sort
```
...
```{index} sorting; selection sort algorithms; selection sort nested loop selection sort
```
...
```{index} sorting; Shell sort nested loop Shell sort algorithms; Shell sort
```
...
```{index} sorting; Quicksort algorithms; Quicksort recursion; Quicksort Quicksort
```
